...
// Type-only imports – for IntelliSense only, no JS output
import type {
  Scene,
  Vector3,
...
export function createCharacterController(scene: Scene) {
  ...
  let characterOrientation: Quaternion = BABYLON.Quaternion.Identity();
  let forwardLocalSpace = new BABYLON.Vector3(0, 0, 1);

  // Create visual capsule mesh
  const h = 1.8;
  const r = 0.6;
  let displayCapsule = BABYLON.MeshBuilder.CreateCapsule(
    "CharacterDisplay",
    { height: h, radius: r },
    scene
  );
  ...
  let characterController: PhysicsCharacterController =
  ...
  const getDesiredVelocity = function (
    ...
    let forwardWorld = forwardLocalSpace.applyRotationQuaternion(
  ...
  // Optionally return controller + mesh if you want to use them later
